...
import { FiUser, FiPhone, FiLock, FiMapPin, FiUserPlus } from 'react-icons/fi';
...
export default function Register() {
  const [form, setForm] = useState({ username: '', phone: '', password: '', address: '' });
    ...
    <div className="auth-page">
      ...
      <motion.div className="auth-card" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="auth-header">
          ...
          <p>Rejoignez la communauté Shifa</p>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label><FiUser size={14} /> Nom d'utilisateur *</label>
            <input className="form-control" value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} placeholder="Votre nom" required />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <input className="form-control" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="Min. 6 caractères" required />
          </div>
          <div className="form-group">
            <label><FiMapPin size={14} /> Adresse</label>
            <input className="form-control" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} placeholder="Votre adresse de livraison" />
          </div>
          <button className="btn btn-primary btn-lg" style={{ width: '100%' }} type="submit" disabled={loading}>
            {loading ? 'Création...' : <><FiUserPlus /> Créer mon Compte</>}
          </button>
        </form>
