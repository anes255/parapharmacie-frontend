...
router.post('/register', async (req, res) => {
    try {
        ...
        if (!phoneRegex.test(telephone.replace(/\s+/g, ''))) {
            return res.status(400).json({
                message: 'Format de téléphone invalide (numéro algérien requis: 0555123456 ou +213555123456)'
            });
        }
        ...
        // Update last connection - NO validation to avoid issues
        try {
...
router.post('/login', async (req, res) => {
    try {
        ...
        // Update last connection - FIXED: Skip validation to avoid phone format issues
        try {
            await User.findByIdAndUpdate(user._id, {
                dernierConnexion: new Date()
            }, { 
            ...
            // Don't fail login if we can't update last connection
            console.log('⚠️ Could not update last connection:', updateError.message);
        }
        ...
        res.json({
            ...
            user: {
                ...
                role: user.role,
                dateInscription: user.dateInscription,
                dernierConnexion: new Date() // Return current time
            }
        });
