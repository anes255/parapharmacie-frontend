...
class PharmacieGaherApp {
    ...
    // FIXED: Load Orders Page with proper null checks
async loadOrdersPage() {
    try {
        console.log('Loading orders page for user:', this.currentUser?.id);
        ...
        try {
            ...
            console.log('Local orders loaded:', userOrders.length);
        } catch (error) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.orders) {
                    ...
                    const localOrderNumbers = userOrders.map(o => o.numeroCommande);
                    const newApiOrders = data.orders.filter(o => !localOrderNumbers.includes(o.numeroCommande));
                    userOrders = [...userOrders, ...newApiOrders];
                    console.log('API orders merged:', data.orders.length);
                }
            }
        } catch (error) {
            console.log('API unavailable, using local orders only:', error);
        }
        
        // Sort orders by date, newest first
        userOrders.sort((a, b) => new Date(b.dateCommande) - new Date(a.dateCommande));
        
        console.log('Total orders to display:', userOrders.length);
        ...
        mainContent.innerHTML = `
            <div class="container mx-auto px-4 py-8 max-w-6xl">
                <div class="text-center mb-8">
                    <h1 class="text-4xl font-bold text-emerald-800 mb-4">Mes Commandes</h1>
                    <p class="text-xl text-emerald-600">Suivi de vos commandes</p>
                </div>
                ...
                ` : `
                    <div class="space-y-6">
                        ${userOrders.map(order => {
                            // FIXED: Add proper null checks for order.client
                            const client = order.client || {};
                            const adresse = client.adresse || 'Adresse non spécifiée';
                            const prenom = client.prenom || '';
                            const nom = client.nom || '';
                            const wilaya = client.wilaya || '';
                            const email = client.email || '';
                            const articles = order.articles || [];
                            
                            return `
                                <div class="bg-white rounded-2xl shadow-lg border border-emerald-200/50 overflow-hidden">
                                    <div class="bg-gradient-to-r from-emerald-50 to-green-50 p-6 border-b border-emerald-200/50">
                                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                            <div>
                                                <h3 class="text-xl font-bold text-emerald-800">Commande #${order.numeroCommande}</h3>
                                                <p class="text-emerald-600">Passée le ${new Date(order.dateCommande).toLocaleDateString('fr-FR')} à ${new Date(order.dateCommande).toLocaleTimeString('fr-FR')}</p>
                                            </div>
                                            <div class="flex flex-col sm:flex-row gap-4">
                                                ...
                                                <div class="text-right">
                                                    ...
                                                    <p class="text-sm text-emerald-600">${articles.length} article(s)</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-6">
                                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                            <div>
                                                ...
                                                <div class="space-y-2 text-sm">
                                                    ...
                                                    <p><strong>Mode de paiement:</strong> ${order.modePaiement || 'Non spécifié'}</p>
                                                </div>
                                            </div>
                                                ...
                                                <div class="space-y-3 max-h-48 overflow-y-auto">
                                                    ${articles.map(article => `
                                                        <div class="flex items-center space-x-3 p-3 bg-emerald-50/50 rounded-lg">
                                                            <img src="${article.image || 'https://via.placeholder.com/48x48/10b981/ffffff?text=' + encodeURIComponent((article.nom || '').substring(0, 2))}" 
                                                                 alt="${article.nom || 'Produit'}" 
                                                                 class="w-12 h-12 object-cover rounded-lg border border-emerald-200">
                                                            <div class="flex-1">
                                                                <h5 class="font-medium text-emerald-800">${article.nom || 'Produit inconnu'}</h5>
                                                                <p class="text-sm text-emerald-600">Quantité: ${article.quantite || 0} × ${article.prix || 0} DA</p>
                                                            </div>
                                                            <div class="text-right">
                                                                <p class="font-medium text-emerald-700">${(article.quantite || 0) * (article.prix || 0)} DA</p>
                                                            </div>
                                                        </div>
                                                    `).join('')}
                                                </div>
                                        ...
                                        <div class="border-t border-emerald-200/50 pt-4 mt-6">
                                            <div class="flex flex-col sm:flex-row sm:justify-between gap-4">
                                                <div class="space-y-1">
                                                    <p class="text-sm text-emerald-600">Sous-total: <span class="font-medium">${order.sousTotal || 0} DA</span></p>
                                                    <p class="text-sm text-emerald-600">Frais de livraison: <span class="font-medium">${order.fraisLivraison || 0} DA</span></p>
                                                </div>
                                                <div class="sm:text-right">
                                                    <p class="text-lg font-bold text-emerald-800">Total: ${order.total || 0} DA</p>
                                                    ${order.commentaires ? `<p class="text-sm text-gray-600 mt-1">Note: ${order.commentaires}</p>` : ''}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
        ...
        mainContent.innerHTML = `
            <div class="container mx-auto px-4 py-8 max-w-6xl">
                <div class="text-center py-16">
                    ...
                    <button onclick="app.loadOrdersPage()" class="bg-red-600 text-white font-bold py-3 px-8 rounded-xl hover:bg-red-700 transition-all">
